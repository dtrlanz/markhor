...
use chat::{ChatParams, ChatResult};
use serde::{Deserialize, Serialize};

const URI: &str = "python stdio plugin"; // Todo: Use correct URI
...
use markhor_core::chat::{ChatError, ChatModel, Completion, Message, MessageRole}; // Assuming this is the trait for plugins
...
impl StdioPythonPlugin {
    ...
    async fn initialize(&self) -> Result<InitializedState, PluginError> {
        ...
        tracing::info!("Plugin '{}' initialized successfully.", self.id);
        Ok(InitializedState {
            venv_python_path,
            plugin_script_path,
        })
    ...
    async fn run_method<Req: Serialize, Resp: for<'de> Deserialize<'de>>(
        &self,
        method: &str,
        params: Req,
        ...
        if !stderr_str.trim().is_empty() {
            ...
            if status.success() {
                tracing::debug!("Plugin '{}' stderr: {}", self.id, stderr_str.trim_end());
            } else {
                tracing::warn!("Plugin '{}' stderr: {}", self.id, stderr_str.trim_end());
            }
        }
        ...
        if !status.success() {
                ...
                match serde_json::from_slice::<PluginResponse<serde_json::Value>>(&stdout_buf) {
                    ...
                    Err(parse_err) => {
                        ...
                        tracing::warn!(
                            ...
                            self.id, status, parse_err, stdout_preview.chars().take(100).collect::<String>()
                        );
                        // Fall through to return ProcessFailed below
                    }
                }
            } else {
                tracing::warn!("Plugin '{}' exited non-zero ({}) and stdout was empty.", self.id, status);
                // Fall through to return ProcessFailed below
            }
            ...
            match response {
                PluginResponse::Success { result } => {
                    tracing::debug!("Plugin '{}' returned success.", self.id);
                    Ok(result)
                },
...
impl ChatModel for StdioPythonPlugin {
    async fn chat(
        &self,
        messages: &[Message],
        model: Option<&str>,
        ...
        tracing::debug!("Calling chat method on plugin '{}'", self.id);
        let converted_messages: Vec<chat::ChatMessage> = messages.iter().map(|m| m.into()).collect();
        ...
        let result: ChatResult = self.run_method("chat", params).await.map_err(|e| e.into())?;
