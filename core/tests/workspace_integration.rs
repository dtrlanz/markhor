...
use markhor_core::storage::{
    ...
    Error,
    ConflictError,
    // You might need to adjust imports based on your module structure
    // e.g., use your_library_name::storage::{Workspace, Document, Folder};
};
...
async fn integration_create_and_open_workspace() {
    ...
    assert!(created_ws.internal_dir_path().exists(), "Internal .markhor directory should exist after create");
    assert!(created_ws.internal_dir_path().is_dir(), "Internal .markhor should be a directory");
    ...
    assert!(opened_ws.internal_dir_path().exists(), "Internal .markhor directory should exist after open");
...
async fn integration_move_document_between_folders() {
    let dir = tempdir().unwrap();
    let ws = Workspace::create(dir.path().to_path_buf()).await.unwrap();
    let folder_a_path = ws.path().join("DirA");
    let folder_b_path = ws.path().join("DirB");
    let original_doc_path = folder_a_path.join("movable.markhor");
    let original_file_path = folder_a_path.join("movable.data");
    let target_doc_path = folder_b_path.join("moved_doc.markhor"); // Moving and renaming

    // 1. Setup initial state
    create_dummy(&folder_a_path, true).await;
    create_dummy(&folder_b_path, true).await;
    let doc = Document::create(original_doc_path.clone()).await.unwrap();
    ...
    assert!(!folder_b_path.join("moved_doc.data").exists());
    ...
    assert!(folder_b_path.join("moved_doc.data").exists(), "Target .data file should exist");
    ...
    let folder_b = folders.iter().find(|f| f.path() == folder_b_path).unwrap();
    let docs_in_b = folder_b.list_documents().await.unwrap();
    assert_eq!(docs_in_b.len(), 1);
    assert_eq!(docs_in_b[0].path(), target_doc_path);

    // 5. Verify listing in old location
    let folder_a = folders.iter().find(|f| f.path() == folder_a_path).unwrap();
    let docs_in_a = folder_a.list_documents().await.unwrap();
...
async fn integration_move_document_causes_conflict() {
    ...
    // Need to reopen doc2 as it was consumed
    let doc2_reopened = Document::open(doc2_path.clone()).await.unwrap();

    // 3. Attempt to move doc1 to doc2's name - should conflict (ExistingFileWouldBeAdopted is less likely than MarkhorFileExists, let's test adopt)
    // Setup: doc1, file named doc2.txt. Try moving doc1 -> doc2.markhor
    let doc1_reopened = Document::open(doc1_path.clone()).await.unwrap();
    let conflicting_file_for_doc2 = ws.path().join("doc2.txt");
    create_dummy(&conflicting_file_for_doc2, false).await;

    let move_result_2 = doc1_reopened.move_to(doc2_path.clone()).await;
...
async fn integration_move_adopts_orphan_conflict() {
    ...
    let source_doc = Document::create(source_doc_path.clone()).await.unwrap();
...
async fn integration_create_document_causes_conflict() {
    ...
    let base_doc_path = ws.path().join("base.markhor");
    let suffix_doc_path = ws.path().join("base.a1.markhor");
    ...
    let create_result_1 = Document::create(base_doc_path.clone()).await;
    ...
    let _base_doc = Document::create(base_doc_path.clone()).await.unwrap();

    // 3. Attempt to create base.a1.markhor - conflicts with existing base.markhor (Rule 3)
    let create_result_2 = Document::create(suffix_doc_path.clone()).await;
    ...
    // 4. Delete base, create suffix, try creating base (Rule 4)
     _base_doc.delete().await.unwrap();
     let _suffix_doc = Document::create(suffix_doc_path.clone()).await.unwrap();
     let create_result_3 = Document::create(base_doc_path.clone()).await;
